<template>
  <div>
    <section
      class="page-header"
      :style="{
            backgroundImage: `url(${GRIDSOME_API_URL + blogGeneral.useBackgroundImage[0].url})`,
            color: blogGeneral.fontColor 
        }"
    >
      ...
      <h1 class="project-name">{{ blogGeneral.blogTitle }}</h1>
      <h2 class="project-tagline">{{ blogGeneral.blogDescribe }}</h2>
      <a
        :href="'https://github.com/' + blogGeneral.githubUsername"
        class="btn"
        target="_blank"
        >GitHub主页</a
      >
      <a
        href="https://github.com/GitHub-Laziji/blogGeneral"
        class="btn"
        target="_blank"
        v-if="!blogGeneral.mini"
        >博客源码</a
      >
    </section>
    <div
        ...
        <el-row>
          <el-col :span="10">
            ...
            >
              ...
              <el-submenu
                index="#webSites"
                v-if="blogGeneral.webSites.split(',').length > 0"
              >
                <template slot="title">其他网站</template>
                <el-menu-item
                  :index="'#webSites-' + index"
                  v-for="(item, index) in blogGeneral.webSites.split(',')"
                  :key="'#webSites' + index"
                  >{{ item }}</el-menu-item
                >
              </el-submenu>
          ...
          <el-col :span="4" :offset="8" style="text-align: right">
            <div style="font-size: 20px; color: #606266; margin-top: 5px">
              <b>{{ blogGeneral.githubUsername }}</b>
            </div>
            <div style="color: #606266">
              <i class="el-icon-location"></i>&nbsp;{{
                blogGeneral.location ? blogGeneral.location : "未填写地址"
              }}
              <br />
            </div>
          </el-col>
          <el-col :span="2" style="text-align: center">
            <img
              v-popover:bigAvatar
              :src="GRIDSOME_API_URL + blogGeneral.avatar[0].url"
              style="
                margin-top: 4px;
                margin-right: 10px;
                width: 52px;
            ...
            <el-popover
              ref="bigAvatar"
              placement="top-start"
              :title="blogGeneral.githubUsername"
              width="200"
              trigger="hover"
            >
              <i class="el-icon-star-on"></i>&emsp;{{ blogGeneral.name }}
              <br />
              <i class="el-icon-location"></i>&emsp;{{ blogGeneral.location }}
              <br />
              <img
                :src="GRIDSOME_API_URL + blogGeneral.avatar[0].url"
...
export default {
  ...
  computed: {
    blogGeneral() {
      return this.$static.blogGeneral.edges[0].node;
    },
  },
};
</script>
